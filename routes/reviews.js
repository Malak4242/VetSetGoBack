...
router.post('/', auth, async (req, res) => {
  try {
    ...
    if (!appointment) {
      return res.status(404).json({ msg: 'Appointment not found or not completed' });
    }
...
async function updateRatings(hospitalId, doctorId) {
  ...
  if (hospitalReviews.length > 0) {
    const avgRating = hospitalReviews.reduce((sum, r) => sum + r.rating, 0) / hospitalReviews.length;
    await Hospital.findByIdAndUpdate(hospitalId, { rating: Math.round(avgRating * 10) / 10 });
  }
  ...
  if (doctorReviews.length > 0) {
    const avgRating = doctorReviews.reduce((sum, r) => sum + r.rating, 0) / doctorReviews.length;
    await Doctor.findByIdAndUpdate(doctorId, { rating: Math.round(avgRating * 10) / 10 });
  }
}
